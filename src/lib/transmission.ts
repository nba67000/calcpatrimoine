...
export function calculerTransmission(inputs: TransmissionInputs): TransmissionResults {
  ...
  if (Math.abs(totalParts - 100) > 0.01) {
    warnings.push(`⚠️ Les parts totalisent ${totalParts.toFixed(1)}% au lieu de 100%`)
  }
  ...
  if (plusValueTotale < 0) {
    warnings.push('⚠️ Les versements dépassent le capital actuel')
  }
  ...
  if (versementsApres70 > 0 && capital757BTotal - abattement757BGlobal > 0) {
    infos.push(`ℹ️ Les versements après 70 ans (${versementsApres70.toLocaleString('fr-FR')}€) sont soumis à l'Article 757 B avec un abattement global de 30 500€ partagé.`)
  }
  
  if (plusValueTotale > 0) {
    infos.push(`ℹ️ Les plus-values générées par les versements après 70 ans sont totalement exonérées de droits.`)
  }
  
  if (beneficiaires.some(b => b.lien === 'conjoint')) {
    infos.push(`ℹ️ Le conjoint/partenaire PACS est totalement exonéré (Loi TEPA 2007).`)
  }
